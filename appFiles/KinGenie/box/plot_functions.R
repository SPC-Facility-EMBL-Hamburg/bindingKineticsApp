...
## Auxiliary function to get axis ticks
...
## Auxiliary function to configure the plot
...
## Auxiliary function to add layout to a subplot
...
plot_plate_info <- function(sample_row, sample_column, sample_type,
  ...
  sample_type_simple <- unlist(sapply(sample_type, sample_type_to_letter))

  sample_matrix <- data.frame(
    sample_row = sample_row,
    sample_column = sample_column,
    sample_type_simple = sample_type_simple
  )
  ...
  fig <- fig |> layout(
    xaxis = list(title = "", tickfont = list(size = font_size)),
    yaxis = list(title = "", tickfont = list(size = font_size), autorange = "reversed"),
    title = list(
      text = paste0("Sample plate layout (", experiment_name, ")"),
...
## Auxiliary function to plot traces
...
plot_traces_all <- function(
  ...
  fig <- plot_list_to_fig("Sensor_traces", plot_list, "Sensor traces", font_size,
    ...
    nrows = 2, share_axis = FALSE
  )

  fig
}

## Function to plot steady-state data
#' @export
...
## Function to plot steady-state residuals
#' @export
...
## Function to plot association and dissociation curves
#' @export
...
plot_interactions <- function(
  ...
  fig <- plot_list_to_fig(
    "Interaction_traces_plot", fig_lst, "Interaction traces",
    font_size, plot_type, plot_width, plot_height, 2, FALSE
  )
...
## Function to plot solution fit residuals
#' @export
plot_interactions_residuals <- function(
  ...
  fig <- plot_list_to_fig(
    "Interaction_traces_residuals_plot", fig_lst, "Residuals",
    font_size, plot_type, plot_width, plot_height, 2, FALSE
  )
...
##  Function to plot association and dissociation fit residuals
#' @export
      ...
      for (j in 1:n_traces) {
        ...
        if (plot_assoc) {
          if (we_have_fitted_curves1 && plot_fit) {
            ...
            fig <- add_trace(fig,
              x = x_temp, y = y_res, type = "scatter", mode = "markers",
...
plot_simulation <- function(
  ...
  for (i in 1:n_pcs) {
    ...
    for (cycle in 1:length(signal_per_cycle_assoc)) {
      ...
      for (ii in 1:length(curves_assoc)) {
        ...
        fig <- add_trace(fig,
          x = as.numeric(time_assoc), y = y, type = "scatter", mode = "markers",
        ...
        if (!is.null(time_disso)) {
          ...
          fig <- add_trace(fig,
            x = as.numeric(time_disso), y = y, type = "scatter", mode = "markers",
            name = paste0(l_conc), color = I(hex_color),
            showlegend = FALSE, marker = list(size = marker_size)
          )
        }
  ...
  fig <- plot_list_to_fig(
    "kingenie_simulation_plot", fig_lst, "Kinetic traces",
...
plot_relaxation_rates <- function(
  ...
  for (i in 1:n_pcs) {
    ...
    fig <- add_trace(fig,
      x = ligand_concs, y = k_obs_dominant, type = "scatter", mode = "markers",
    ...
    fig <- add_trace(fig,
      x = ligand_concs, y = k_obs_non_dominant, type = "scatter", mode = "markers",
...
##  Function to plot the relaxation rates versus ligand concentration for multiple protein concentrations
#' @export
plot_many_relaxation_rates <- function(
  ...
  fig <- plot_list_to_fig(
    "Dominant_relaxation_rates_plot", fig_lst, "Dominant relaxation rates",
    font_size, plot_type, plot_width, plot_height, 2, FALSE
  )
...
diagnostic_plot <- function(
  ...
  for (exp in unq_exp) {
    ...
    if (add_fit) {
      ...
      fig <- add_trace(fig,
        x = x_fit, y = y_fit, type = "scatter", mode = "lines",
  ...
  fig <- plot_list_to_fig(
    "Observed_constants_plot", fig_lst, "Observed constant", font_size,
...
##  Auxiliary function to create a plotly output with a fixed height of 600px
#' @export
plotlyOutput600px <- function(outputId, ...) {
  plotlyOutput(outputId, height = "600px", ...)
}
